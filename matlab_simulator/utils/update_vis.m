...
if nargin > 3
    ...
    obs_tf = [cos(vis.obs(3)) -sin(vis.obs(3)) vis.obs(1);
              sin(vis.obs(3)) cos(vis.obs(3)) vis.obs(2);
              0 0 1];
    ...
    if nargin > 4
        ...
        for j=1:size(path,2)-1
            vis.path_plot(j) = plot([path(1,j,i) path(1,j+1,i)],[path(2,j,i),path(2,j+1,i)],'Color',vis.path_color);
        end
    end
end
