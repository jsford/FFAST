...
if abs(v_x) < 0.1 && abs(v_y) < 0.1
    alpha_f = 0;
    alpha_r = 0;

% perfect side slip, no longitudinal vel
elseif v_x == 0
    alpha_f = pi/2*sign(v_y)-delta;
    alpha_r = pi/2*sign(v_y);
...
v_x_dot = ma_x/vehicle.m + yaw_rate*v_y -0.25*v_x;
v_y_dot = ma_y/vehicle.m - yaw_rate*v_x -0.25*v_y;
