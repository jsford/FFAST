...
<br>Installation instructions can be found [here](https://lcm-proj.github.io/build_instructions.html).
<br>
<br>Ensure you have these required packages:
* build-essential
* libglib2.0-dev
* openjdk-8-jdk
* python-dev

<br>Download LCM from their [Downloads](https://github.com/lcm-proj/lcm/releases) page.
<br>From a terminal, run the following commands: 
```bash
$ unzip lcm-X.Y.Z.zip
$ cd lcm-X.Y.Z
$ ./configure
...
<br>The PodsTool provides templates for building pods of a few different types.
<br>Installation instructions can be found [here](https://sourceforge.net/p/pods/wiki/PodsTool/).
<br>
<br>Ensure you have subversion and cmake installed
```bash
...
<br>
<br>Clone the flexiport repository from [here](https://github.com/gbiggs/flexiport).
<br>Edit the CMakeLists.txt file and turn the build examples, python bindings and documentation options off.
```bash
$ mkdir build
$ cd build
$ cmake ..
...
<br>
<br>Clone the HokuyoAIST repository from [here](https://github.com/gbiggs/hokuyoaist)
<br>Edit the CMakeLists.txt file and turn the build examples, python bindings and documentation options off.
```bash
$ mkdir build
$ cd build
...
<br>You can verify that it is enabled by checking that the line `CONFIG_USB_ACM=y` exists on the config file. (Instruction was obtained from [here](https://github.com/datlife/jetson-car/issues/7))
<br>Build the kernel after that.
```bash
$ ./makeKernel.sh
$ ./copyImage.sh
```
...
<br>Download the Arduino IDE installer for LinuxARM from [here](https://www.arduino.cc/en/Main/Software).
<br>Extract the installation directory and move it into the /opt directory (the /opt directory is reserved for all the software and add-on packages that are not part of the default installation).
```bash
...
$ ./install.sh
```
<br>After installation, several errors may occur while launching the Arduino IDE using the `./arduino` command.
<br>
<br>If the error `/opt/arduino-1.8.5/java/bin/java: No such file or directory` is encountered, 32-bit Java needs to be installed. (Installation instructions were obtained from [here](https://github.com/openhab/openhabian/issues/57))
...
<br>Verify that it has been successfully configured by running
```bash
...
<br>If the error `java.lang.UnsatisfiedLinkError: <some_library> : wrong ELF class: ELFCLASS64` is encountered, download the library with armhf architecture.
<br>For example, error: `java.lang.UnsatisfiedLinkError: /opt/arduino-1.8.5/java/lib/arm/libawt_xawt.so: libXrender.so.1: wrong ELF class: ELFCLASS64`
<br>Download libXrender by
```bash
$ sudo apt-get install libxrender-dev:armhf
```
<br>
<br>If the error `can't load com.sun.java.swing.plaf.gtk.GTKLookAndFeel` is encountered, the arduino startup script needs to be edited. (Solution was obtained from [here](http://forum.arduino.cc/index.php?topic=26770.0))
<br>Remove the definition `"-Dswing.defaultlaf=com.sun.java.swing.plaf.gtk.GTKLookAndFeel"` from `JAVA_OPTIONS`.
<br>
...
<br>Set the udev rule for the Teensy device
```bash
$ cd /etc/udev/rules.d
...
<br>
<br>When running the installer, several errors may again be encountered. These are the same errors regarding the 32-bit libraries that cannot be found, hence again just download the corresponding libraries in armhf architecture.
<br>If the installation is successful but uploading through the Arduino IDE fails due to the Teensy loader, it is the same error that still exists.
<br>Run the teensy executable:
```bash
...
<br>First, open jssc-2.8.0.jar that is in the /opt/arduino-1.8.5/lib directory using an archive manager.
<br>In the .jar file there is a libs/linux/ directory, copy the libjSSC-2.8_armhf.so into the ~/.jssc/linux folder (you can use the terminal to navigate to that folder and enter the command `nautilus .` to open in files explorer). Create the .jssc folder if it doesn't exist.
<br>If the ~/.jssc/linux folder already has a libjSSC-2.8_armsf.so delete it. (notice the sf.so vs hf.so for the file we extracted.)
<br>Right click the libjSSC-2.8_armhf.so and click Make Link.
<br>Rename the link to libjSSC-2.8_armsf.so (again notice the sf.so)
<br>The next time you open the Arduino IDE and the Serial Monitor from there it should no longer print an exception and it should work.
<br>

## Installing packages with no installation candidate
On Jetson TX2, many packages may not have their installation candidates.
<br>This may be solved by
```bash
